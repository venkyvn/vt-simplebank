...
func (s *Server) createTransfer(context *gin.Context) {
	...
	if !s.validAccount(context, req.FromAccountID, req.Currency) {
		return
	}
	if !s.validAccount(context, req.ToAccountID, req.Currency) {
		return
	}
	...
	result, err := s.store.TransferTx(context, arg)
	if err != nil {
